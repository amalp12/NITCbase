...
int Algebra::join(char srcRelation1[ATTR_SIZE], char srcRelation2[ATTR_SIZE], char targetRelation[ATTR_SIZE], char attribute1[ATTR_SIZE], char attribute2[ATTR_SIZE]) {
    ...
    int response1 = AttrCacheTable::getAttrCatEntry(srcRelId1, attribute1, &attrCatEntry1);
    int response2 = AttrCacheTable::getAttrCatEntry(srcRelId2, attribute2, &attrCatEntry2);
    ...
    // present in srcRelation2 (getAttrCatEntry() returned E_ATTRNOTEXIST)
        // return E_ATTRNOTEXIST.
    if(response1 == E_ATTRNOTEXIST || response2 == E_ATTRNOTEXIST) {
        return E_ATTRNOTEXIST;
    }
    ...
    response1 = RelCacheTable::getRelCatEntry(srcRelId1, &relCatEntry1);
    response2 = RelCacheTable::getRelCatEntry(srcRelId2, &relCatEntry2);
    ...
    AttrCatEntry tempAttrCatEntry1, tempAttrCatEntry2;


    for(int i = 0; i < numOfAttributes1; i++) {
        response1 = AttrCacheTable::getAttrCatEntry(srcRelId1, i, &tempAttrCatEntry1);
        ...
        for(int j = 0; j < numOfAttributes2; j++) { 
            response2 = AttrCacheTable::getAttrCatEntry(srcRelId2, j, &tempAttrCatEntry2);
            if(response2 != SUCCESS) {
                printf("Invalid Attribute ID.\n");
                exit(1);
            }

            if(strcmp(tempAttrCatEntry1.attrName, tempAttrCatEntry2.attrName) == 0) {
    ...
    if(attrCatEntry2.rootBlock == -1) {
        int response = BPlusTree::bPlusCreate(srcRelId2, attrCatEntry2.attrName);
        if(response != SUCCESS) {
            return response;
        }
    ...
    for(int i = 0; i < numOfAttributes1; i++) {
        ...
        strcpy(targetRelAttrNames[targetRelAttrIndex], tempAttrCatEntry1.attrName);
        targetRelAttrTypes[targetRelAttrIndex] = tempAttrCatEntry1.attrType;
        targetRelAttrIndex++;
    }

    for(int i = 0; i < numOfAttributes2; i++) {
        response2 = AttrCacheTable::getAttrCatEntry(srcRelId2, i, &tempAttrCatEntry2);
        if(response2 != SUCCESS) {
            printf("Invalid Attribute ID.\n");
            exit(1);
        }
        ...
        strcpy(targetRelAttrNames[targetRelAttrIndex], tempAttrCatEntry2.attrName);
        targetRelAttrTypes[targetRelAttrIndex] = tempAttrCatEntry2.attrType;
        targetRelAttrIndex++;
    }


    // create the target relation using the Schema::createRel() function
    response1 = Schema::createRel(targetRelation, numOfAttributesInTarget, targetRelAttrNames, targetRelAttrTypes);
    ...
    // reset search index srcRelId1
    response1 = RelCacheTable::resetSearchIndex(srcRelId1);

    response2 = AttrCacheTable::resetSearchIndex(srcRelId1, attribute1);
    ...
    while (BlockAccess::project(srcRelId1, record1) == SUCCESS) {
        ...
        // using RelCacheTable::resetSearchIndex()
        response1 = RelCacheTable::resetSearchIndex(srcRelId2);

        // reset the search index of `attribute2` in the attribute cache
        // using AttrCacheTable::resetSearchIndex()
        response2 = AttrCacheTable::resetSearchIndex(srcRelId2, attribute2);
        ...
        while (BlockAccess::search(
            ...
            // BlockAccess::insert()
            response1 = BlockAccess::insert(targetRelId, targetRecord);

            
            /* insert fails (insert should fail only due to DISK being FULL) */
            if(response1 != SUCCESS) {
